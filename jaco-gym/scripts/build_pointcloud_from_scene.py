...
def get_intrinsic(args,simulated=False):
    if simulated:
        Fx=336.3354187
        Fy=336.3354187
        Cx=238.20429993
...
def image_to_point_clouds(color, depth, args, simulated=False, plot=True):
    np_intrinsic, homography = get_intrinsic(args, simulated=simulated)
...
def run_sequence():
    ...
    for all_world_coord in all_world_coords:
        if x_data is None:
            ...
            z_data += all_world_coord[2]
    """
    x_data=all_point_cloud[:,0]
    y_data=all_point_cloud[:,1]
    z_data=all_point_cloud[:,2]
